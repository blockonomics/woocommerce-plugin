...
class Blockonomics
{
    ...
    public function get_parameterized_wc_page_url($params = array()){
        // $order_url = WC()->api_request_url('WC_Gateway_Blockonomics');
        
        $order_url = wc_get_page_permalink('bcheckout');
    ...
    // Returns WC api endpoint of order adding the given extra parameters
    public function get_parameterized_wc_api_url($params = array()){
        $order_url = WC()->api_request_url('WC_Gateway_Blockonomics');
        if(is_array($params) && count($params)>0){
            foreach ($params as $param_name => $param_value) {
                $order_url = add_query_arg($param_name, $param_value, $order_url);
    ...
    public function get_order_checkout_url($order_id){
        ...
        if (count($active_cryptos) > 1) {
            $order_url = $this->get_parameterized_wc_page_url(array('select_crypto'=>$order_hash));
        } elseif (count($active_cryptos) === 1) {
            $order_url = $this->get_parameterized_wc_page_url(array('show_order'=>$order_hash, 'crypto'=> array_keys($active_cryptos)[0]));
        } else if (count($active_cryptos) === 0) {
            $order_url = $this->get_parameterized_wc_page_url(array('crypto'=>'empty'));
        } 
        return $order_url;
    }
    ...
    public function get_checkout_script($context, $template_name) {
        ...
        if ($template_name === 'checkout') {
            ...
            $script = "const blockonomics_data = '" . json_encode( array (
                ...
                'get_order_amount_url' => $this->get_parameterized_wc_api_url(array('get_amount'=>$order_hash, 'crypto'=>  $context['crypto']['code'])),
